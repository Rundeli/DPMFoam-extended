...
{
    scalar fraction01 = setFraction01(mesh,position,trii);
...
//fraction01 obtained from cumulated triface area fraction of the triface closest to the position given
//fraction01 is only used to determine which processor contains the triface in which the particle to be 
//insert and have nothing to do with the precise location of the inserting particle
//the closest triface to the given position is the triface on the patch whose face centre's distance 
//being the smallest  
Foam::scalar Foam::PeriodPatchInjectionBase::setFraction01
( 
    const fvMesh& mesh,
...
)
{
    ...
    //定义一个较大的最小距离，防止在循环中不被改变
    scalar smallDist = 1e5;

    //遍历triFace中每个三角面
    for(const auto& t : triFace_)
    {
        //计算输入位置，即粒子在outflow边界面上的位置与三角面体心间距离
        scalar pDist = t.centre(points).distSqr(position);
        //如果小于之前的最小距离则设为当前最小距离并将trii设为当前三角面label
        if(pDist < smallDist)
        {
            smallDist = pDist;
            trii = t.index();
    ...
    scalar cumulativeTriAreaTo = triCumulativeMagSf_[trii+1];
    //返回该triFace处的累计三角面与patch总面积之比,即fraction01
    return (cumulativeTriAreaTo/patchArea_);
}
